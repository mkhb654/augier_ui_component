...
export default function TemplateSelection() {
    const { selectedTemplate, setSelectedTemplate, files, addFiles, removeFile } = useContext(AppContext); // Use context
    ...
    const handleFileUpload = (e) => {
        ...
        if (uploadedFiles.length !== e.target.files.length) {
            alert('Some files were not supported and were not added. Please upload only PDF or Word documents.');
        }

        addFiles(uploadedFiles); // Use context to add files
                            ...
                            {files.length > 0 && (
                                <div className="flex flex-wrap gap-2 mt-2"> {/* Reduced gap */}
                                    {files.map((file, index) => {
                                        const FileIcon = file.type.includes('pdf') ? FaFilePdf : FaFileWord;
                                        return (
                                            <div key={index} className="flex items-center space-x-1 border border-gray-200 rounded-md p-1 bg-gray-50"> {/* Reduced padding */}
                                                ...
                                                <button onClick={() => removeFile(index)} className="focus:outline-none">
